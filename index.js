 ...
 */
const validUserName = users.every(user => true)
console.log(validUserName);
const ValidUserEmail = users.every(user => /[^@]+@[^\.]+\..+/.test(user.email))
 ...
 */
const ipAddressA = users.filter(({ip_address}) => ip_address.split('.')[0] <128).length;
console.log(ipAddressA);
/**
 * Exercise 03
 * 
 * Find the position in the array of the first  user object that has a Class B IP address
 */
const ipAddressB = users.findIndex(({ip_address}) => { 
    const firstOctect = ip_address.split('.')[0];
    return firstOctect >= 128 && firstOctect <= 191; 
});

console.log("Position of the first user with a Class B IP address:", ipAddressB);
 ...
 */
const userWithSoundcloudEmail = users.find(user => user.email.includes('soundcloud'));

console.log("User object with 'soundcloud' email address:", userWithSoundcloudEmail);
